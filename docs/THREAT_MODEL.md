...
|---|---|---|
| Lost device or copied database | AES-256-GCM secrets; random DEK; passphrase-wrapped envelope protected again by a device-bound Keystore/Keychain key; automatic/background lock. | A live unlocked process, coercion, or compromised OS can expose data. Losing the device key makes the local database unusable; recovery then requires a valid encrypted cloud backup and its passphrase. |
| Passphrase guessing | 16-character minimum, common-passphrase blocklist, salted Argon2id, and a persistent 30-second exponential delay after five local failures (max 15 minutes). | User-chosen passphrases may still be weak or reused. An extracted cloud recovery envelope enables offline guessing outside the app's local throttle. |
...
| Cloud-provider disclosure | Entire V2 state encrypted with the DEK; the 16+ character blocked-list-checked vault passphrase derives the cloud recovery wrapper; opaque HMAC namespace filenames; app-private provider scope. | Size/timing/count correlation and offline passphrase guessing remain possible. |
| Malicious cloud content | AES-GCM; immutable parent graph; bounded streaming/files/records/JSON; full authentication/decryption before local replacement; fallback to healthy authenticated snapshots. | A provider can withhold/delete data or present a consistent older history to an isolated device. |
...
| Different vault in one provider account | Vault-ID mismatch blocks automatic upload; explicit verified restore/replace/merge/account-switch choices; phone-owner authentication for destructive choices; replacement upload is download-verified before cleanup. | A mistaken but authenticated choice can still discard the selected local or cloud history; provider retention may preserve deleted files. |
...
| Cloud deletion mistake | Current namespace requires the vault passphrase plus `DELETE CLOUD VAULT`; delete-all separately reauthorizes and requires `DELETE ALL CLOUD VAULTS`. | Provider retention policies may retain deleted data; delete-all intentionally affects every PassCrate vault in that app folder. |
